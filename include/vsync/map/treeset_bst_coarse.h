...
#ifndef VSYNC_TREESET_BST_COARSE_H
#define VSYNC_TREESET_BST_COARSE_H

/*******************************************************************************
 * @file treeset_bst_coarse.h
 * @ingroup linearizable
 * @brief This implementation of treeset uses unbalanced binary search tree
 * (BST) and coarse-grained locking.
 *
 * A treeset is a binary search tree with nodes that are ordered by unique keys.
 *
 * Available implementations share the same interface, but differ in three
 * orthogonal aspects:
 * * balancing scheme: unbalanced binary search tree (BST) or balanced red-black
 * tree (RB),
 * * synchronisation scheme: so far only coarse-grained locking and fine-grained
 * locking,
 * * allocation scheme: so far only malloc-like allocation.
 *
 * All implementations use external trees, where values are stored in external
 * nodes (leaves) of the tree, and internal nodes are used only for routing.
 *
 * For lock-based implementations one need to choose desired lock
 * implementation:
 ...
 *
 * @example
 * @include eg_treeset_bst_coarse.c
 ******************************************************************************/

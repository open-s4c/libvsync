...
void
_free_fun(smr_node_t *node, void *args)
{
    V_UNUSED(args);
    my_node_t *nn = V_CONTAINER_OF(node, my_node_t, smr_node);
    free(nn);
}
...
_retire_fun(vrbtree_node_t *node, void *args)
{
    V_UNUSED(args);
    my_node_t *nn = V_CONTAINER_OF(node, my_node_t, embed);
    ismr_retire(&nn->smr_node, _free_fun, false);
}
...
reader_add(void *arg)
{
    ...
    for (vsize_t j = 0; j < NCHECKS; ++j) {
        ...
        ismr_enter(id + 1);
        while (!vrbtree_get(&g_tree, g_keys[g_perm_add[idx]])) {
            ++cnt;
        }
        ASSERT(idx == 0 || vrbtree_get(&g_tree, g_keys[g_perm_add[idx - 1]]));
        ismr_exit(id + 1);
    }
...
reader_rem(void *arg)
{
    ...
    for (vsize_t j = 0; j < NCHECKS; ++j) {
        vsize_t idx = g_read_check[id][j];
        ismr_enter(id + NREADERS + 1);
        while (vrbtree_get(&g_tree, g_keys[g_perm_rem[idx]])) {
            ++cnt;
        }
        ASSERT(idx == 0 || !vrbtree_get(&g_tree, g_keys[g_perm_rem[idx - 1]]));
        ismr_exit(id + NREADERS + 1);
    }
    printf("Reader rem %lu: %lu\n", id, cnt);

    ismr_dereg(id + NREADERS + 1);
    return NULL;
}
...
main(void)
{
    ...
    for (vsize_t iter = 0; iter < NITERS; ++iter) {
        ...
        for (vsize_t i = 0; i < NKEYS; ++i) {
            ...
            ismr_enter(MAIN_TID);
            ASSERT(vrbtree_get(&g_tree, key) == &node->embed);
            ASSERT(vrbtree_remove(&g_tree, key));
            ASSERT(!vrbtree_get(&g_tree, key));
            ismr_exit(MAIN_TID);
        }

...
/*****************************************************************************
 * Multi-thread Test: vatomic64_await_mask_eq
 *****************************************************************************/
static inline void *
mt_atomic_u64_await_mask_eq_run(void *args)
{
    vsize_t tid       = (vsize_t)(vuintptr_t)args;
    vuint64_t mask    = (vuint64_t)tid;
    vuint64_t new_val = (vuint64_t)(tid + 1);

    vuint64_t cur = vatomic64_await_mask_eq(&g_shared, mask, mask);
    ASSERT(cur == mask);
    vatomic64_write(&g_shared, new_val);
    V_UNUSED(cur);
    return NULL;
}
static inline void
mt_atomic_u64_await_mask_eq(void)
{
    vatomic64_init(&g_shared, 0);
    launch_threads(MAX_THREADS, mt_atomic_u64_await_mask_eq_run);
    vuint64_t cur = vatomic64_read(&g_shared);
    ASSERT(cur == MAX_THREADS);
    V_UNUSED(cur);
}
/*****************************************************************************
 * Multi-thread Test: vatomic64_await_mask_neq
 *****************************************************************************/
static inline void *
mt_atomic_u64_await_mask_neq_run(void *args)
{
    vsize_t tid    = (vsize_t)(vuintptr_t)args;
    vuint64_t mask = (vuint64_t)tid;

    if (tid == 0) {
        vatomic64_write(&g_shared, 0);
    } else {
        vuint64_t cur = vatomic64_await_mask_neq(&g_shared, mask, mask);
        ASSERT(cur == 0);
        V_UNUSED(cur);
    }
    return NULL;
...
static inline void
mt_atomic_u64_await_mask_neq(void)
{
    vatomic64_init(&g_shared, VUINT64_MAX);
    launch_threads(MAX_THREADS, mt_atomic_u64_await_mask_neq_run);
    vuint64_t cur = vatomic64_read(&g_shared);
    ASSERT(cur == 0);
    V_UNUSED(cur);
}
...
main(void)
{
    ...
    mt_atomic_u64_await_mask_eq();
    mt_atomic_u64_await_mask_neq();
